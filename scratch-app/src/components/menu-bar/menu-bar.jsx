...
class MenuBar extends React.Component {
    ...
    componentDidMount () {
        ...
        if (this.props.vm) {
            window.__scratch_vm = this.props.vm;
        }
    }
    ...
    handleClickNew () {
        const projectName = window.prompt(
            this.props.intl.formatMessage({
                ...
                description: 'Prompt for new LimX project name',
                id: 'limx.project.namePrompt'
            })
        );
        const cleanName = projectName && projectName.trim();
        if (!cleanName) {
            ...
            fetch('/project/create_empty', {
                ...
                .then(data => {
                    if (data.result === 'success') {
                        this.props.onClickNew(this.props.canSave && this.props.canCreateNew, cleanName);
                        window.dispatchEvent(new CustomEvent('limx-projects-changed', {
                            detail: {name: data.name}
                        }));
                    } else {
                        // eslint-disable-next-line no-alert
                        alert(data.message || 'Project already exists, please choose another name.');
                    }
                })
                .catch(err => {
                    // eslint-disable-next-line no-alert
    ...
    render () {
        return (
            ...
            >
                <div className={styles.limxBrand}>
                    <span className={styles.limxBrandMark}>LIMX</span>
                    <span className={styles.limxBrandTitle}>
                        <FormattedMessage
                            defaultMessage="LimX Robot Programming"
                            description="LimX Robot local Scratch editor title"
                            id="limx.menuBar.title"
                        />
                    </span>
                </div>
                <div className={styles.limxMenuActions}>
